...
return;var mm=wnd.minMaxify={shop:'fear-of-god.myshopify.com',cart:null,cartLoadTryCount:0,customer:wnd.minMaxifyCustomer,feedback:function(msg){},guarded:function(f,pref){return function(){try{var r0=pref&&pref.apply(this,arguments),rv=f.apply(this,arguments);return pref?(rv||r0):rv;}
catch(ex){console.error(ex);}}}};var path=wnd.location.pathname;mm.guarded(function(){var $=wnd.jQuery,doc=wnd.document;var messages={"CART_UPDATE_MSG":"Cart needs to be updated","CART_UPDATE_MSG2":"Cart contents have changed, you must click \"Update cart\" before proceeding.","INTRO_MSG":"Cannot place order, conditions not met: \n\n","MAX_SUBTOTAL_MSG":"Must have at most {{CartMaxAmount}} in total.","MIN_SUBTOTAL_MSG":"Must have at least {{CartMinAmount}} in total.","NOT_VALID_MSG":"Order not valid","PROD_MAX_MSG":"{{ProductName}}: Must have at most {{ProductMaxQuantity}} of this item.","PROD_MIN_MSG":"{{ProductName}}: Must have at least {{ProductMinQuantity}} of this item.","PROD_MULT_MSG":"{{ProductName}}: Quantity must be a multiple of {{ProductQuantityMultiple}}.","TOTAL_ITEMS_MAX_MSG":"Must have at most {{CartMaxQuantity}} items total.","TOTAL_ITEMS_MIN_MSG":"Must have at least {{CartMinQuantity}} items total.","TOTAL_ITEMS_MULT_MSG":"Must have a multiple of {{CartQuantityMultiple}} items total.","VERIFYING_MSG":"Verifying"};var checkoutSelector="[name='checkout'], [aria-label='Apple Pay'], [name='goto_pp'], .amazon-payments-pay-button > img, [href='/checkout'], [type=submit][value=Checkout], [onclick='window.location\\=\\'\\/checkout\\''], form[action=\\/checkout] [type=submit], .carthook_checkout";var updatableCheckoutBtnSelector='[name="checkout"], input.carthook_checkout';var selCartQtyAdjust='.cart-item-decrease, .cart-item-increase, .js--qty-adjuster, .js-qty__adjust, .minmaxify-quantity-button, .numberUpDown > *, .cart-item button.adjust';var checkoutFrames=".additional-checkout-buttons, .dynamic-checkout__content, .cart__additional_checkout, .additional_checkout_buttons, .paypal-button-context-iframe, .additional-checkout-button--apple-pay, .additional-checkout-button--google-pay";var dynamicCheckoutFrames='div.shopify-payment-button';var selMinField=".minmaxify-minfield";var selQantityField="input[name=quantity]";var selCartQtyField='input[name^="updates["], .cart__qty-input';var eXHR;var limitsJson={"minorder":"","maxorder":"","mintotalitems":"","maxtotalitems":"","multtotalitems":"","itemmin":"","itemmax":"","itemmult":"","weightmin":"","weightmax":"","overridesubtotal":"","lockchange":true,"embedjq":false,"syncload":true,"productLimits":{"air-fear-of-god-moc-bone":{"min":"","max":"1","multiple":"","combine":true,"name":"Air%20Fear%20of%20God%20Moc"},"air-fear-of-god-raid-bone":{"min":"","max":"1","multiple":"","combine":true,"name":"Air%20Fear%20of%20God%20Raid"},"740237574502":{"min":"","max":"1","multiple":"","combine":true,"name":"Air%20Fear%20of%20God%20Tear%20Away%20Pant"},"air-fear-of-god-tear-away-pants-god-grey":{"min":"","max":"1","multiple":"","combine":true,"name":"Air%20Fear%20of%20God%20Tear%20Away%20Pants"},"air-fear-of-god-tear-away-pants":{"min":"","max":"1","multiple":"","combine":true,"name":"Air%20Fear%20of%20God%20Tear%20Away%20Pants"},"maxfield-slip-on-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"Black%20Maxfield%20Style%2047%20Vans"},"white-black-slip-on-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"Bone/Black%20Style%2047%20Vans"},"marshmallow-era-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"FOG%20%2795%20Era%202.0%20Vans"},"mountain-edition-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"FOG%20Mountain%20Edition%20Vans"},"red-corduroy-era-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"FOG%20Red%20Corduroy%20%2795%20Era%20Vans"},"red-black-slip-on-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"Red/Black%20Style%2047%20Vans"},"red-suede-slip-on-vans":{"min":"","max":"2","multiple":"","combine":true,"name":"Red%20Style%2047%20Vans"}},"groupLimits":[],"skus":[]};var popupCount=0;var limitsShown,isValid,qtyChanged,vstart;var originalSubmitText;var errorMessage=[];function now(){return new Date().getTime()}
...
return itm;}
function setCheckoutMsg(t,addClass,remClass){$first(updatableCheckoutBtnSelector,function(c){c.value=t;var cl=c.classList;if(cl&&addClass)
...
$on(c,event,mm.guarded(f));else{c['on'+event].onmousedown=mm.guarded(function(e){if(f())
...
function updateVerificationState(forcePopup){showLimits();if(!hookCheckoutBtn()&&!forcePopup)
...
wnd.getLimits=updateVerificationState;wnd.mmIsEnabled=function(){return isValid};mm.handleCartUpdate=function(cart,add,ignoreEmpty){if(typeof cart=="string")
cart=JSON.parse(cart);if(add||JSON.stringify(mm.cart)!=JSON.stringify(cart)){if(!add){if(ignoreEmpty&&!cart.item_count&&mm.cart)
return;mm.cart=cart;}else{var item=cart;cart=mm.cart||{total_price:0,total_weight:0,items:[],item_count:0};for(var i=cart.items.length-1;i>=-1;--i){if(i>=0){var prev=cart.items[i];if(prev.id!=item.id)
...
t=t.substring(t.indexOf('{'),t.length-1);f=mm.handleCartUpdate(t,0,ignoreEmpty);}else if(url.indexOf("/cart/add.js")!=-1){f=mm.handleCartUpdate(t,1,ignoreEmpty);}else if(method=="GET"&&(!isCartPage()||url=='/cart')){updateVerificationState();f=1;}
if(eXHR)
...
return;errorMessage=[formatT("INTRO_MSG")];bindOnce('.js-drawer-open-right','click',function(){setTimeout(updateVerificationState,500);});initPopups();updateVerificationState();if(wnd.trekkie){var oldTrack=trekkie.track;trekkie.track=function(e){if(e=='Viewed Product Variant'){limitsShown=0;setTimeout(mm.guarded(showLimits),0);}
